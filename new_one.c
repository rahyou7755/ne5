#include <stdio.h>
#include <malloc.h>
//#include<stdlib.h>

typedef struct _mem_node_t
{
	void * mem_pointer;
	char * file_name;
	char * function;
	int line;
	int size;
	int flag;

} mem_node_t ;

#define MAX_MEM_PTRS 1024   //size
mem_node_t mem_node[MAX_MEM_PTRS];//array of structure

/*add_mem_node - It allocates memory using malloc()/calloc() and saves the context to the one element of mem_node array and marks flag as in use*/

void *add_mem_node(int size,char * file_name,int line,char * function)
{                                                              
	int i = 0;
	int ret = -1;

	void *pointer = malloc(size);//creating memory

	for(i = 0;MAX_MEM_PTRS; i++) {
		if(mem_node[i].flag == 0) {//initaial flag will be zero
			mem_node[i].flag = 1;
			mem_node[i].mem_pointer = pointer;
			mem_node[i].file_name = file_name;
			mem_node[i].function = function;
			mem_node[i].line = line;
			mem_node[i].size = size;
			ret = 0;
			break;
		}
	}
	return pointer;
}

void del_mem_node(void * mem_pointer)
{

	int i = 0;
	int ret = -1;

	for(i = 0; MAX_MEM_PTRS; i++) {
		if(mem_node[i].mem_pointer == mem_pointer) {//if match is found then we are deleting the data by initializing with 0.
			mem_node[i].flag = 0;
			mem_node[i].mem_pointer = NULL;
			mem_node[i].file_name = 0;
			mem_node[i].function = 0;
			mem_node[i].line = 0;
			mem_node[i].size = 0;
			ret = 0;
			free(mem_pointer);
			break;
		}
	}
	return;
}

void show_mem_stat(void)
{
	int i = 0;
	int ret = 0;

	for(i = 0; MAX_MEM_PTRS; i++) {
		if(mem_node[i].flag == 1) {
			printf("0x%p of %d bytes allocated"
					" from %s:%d in %s() is not freed\r\n",
					mem_node[i].mem_pointer,
					mem_node[i].size,
					mem_node[i].file_name,
					mem_node[i].line,
					mem_node[i].function);
			ret = 1;
		}
	}
	if(ret == 0) {
		printf("no memory leak detected! ");
	}
	return;
}

#undef malloc
#define malloc(size) add_mem_node(size, \
		__FILE__, \
		__LINE__, \
		(char*)__FUNCTION__)
#undef free
#define free(_p) del_mem_node(_p)

/*user code starts here - We are detecting leak here*/
/*Please note we do not need to modify malloc/free calls*/

void job1(void)
{
	void * p = malloc(4);
	printf("Job 1\r\n");
	free(p);
}

void job2(void)
{
	void * p = malloc(4);
	printf("Job 2\r\n");
	 free(p);
}

void job3(void)
{
	void * p = malloc(4);
	printf("Job 3\r\n");
	//free(p);
	/*free(p);*/ /* << memory leak added here*/
}

//int main(int argc, char *argv[])
int main()
{
	printf("Detect memory leak using C\r\n");
	job1();
	job2();
	job3();
	show_mem_stat();
	return 0;
}
/*user code ends here*/
